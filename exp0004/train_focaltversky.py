...
class LitSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat = self.model(x)
        #loss = self.dice(y_hat, y) + self.bceloss(y_hat, y) + self.focalloss(y_hat, y)
        loss = self.bceloss(y_hat, y) + self.focaltversky(y_hat, y, smooth=1, alpha=0.25, beta=0.75, gamma=1.0)
