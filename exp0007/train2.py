...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.focalloss = smp.losses.FocalLoss(mode='binary')
        #self.diceloss = smp.utils.losses.DiceLoss(activation='sigmoid')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.bceloss(y_hat[:,0,:,:].unsqueeze(dim=1), y)
        dice = 1-self.dice((torch.sigmoid(y_hat[:,0,:,:].unsqueeze(dim=1))>0.5).float(), y)
        dice_bound = 1-self.dice((torch.sigmoid(y_hat[:,1,:,:].unsqueeze(dim=1))>0.5).float(), y_bound)
